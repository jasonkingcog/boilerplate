...
# Attach the customer-managed policy by name and path.
# The policy must already exist in each target account under the same name/path.
...
resource "aws_ssoadmin_account_assignment" "this" {
  ...
  # The policy attachment must exist before assignments are created.
  depends_on = [aws_ssoadmin_customer_managed_policy_attachment.this]
}
